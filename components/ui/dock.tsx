  ...
  return (
    ...
    >
      <motion.div
        ...
        // centered regardless of direction: left-1/2 + -translate-x-1/2, not a logical inset,
        // since it centers on the physical midpoint either way
        className={cn(
          "absolute bottom-4 left-1/2 flex h-14 -translate-x-1/2 transform items-end gap-3 rounded-xl p-2 bg-opacity-90",
...
function DockCardInner({ src, id, children }: DockCardInnerProps) {
  ...
  return (
    <span className="relative z-0 flex h-full w-full items-center justify-center overflow-hidden rounded-md">
      ...
      <motion.img
        className="absolute z-10 translate-y-2.5 scale-125 opacity-40 blur-md filter"
        src={src}
        alt=""
      />
  ...
  return (
    <div className="flex flex-col items-center gap-1" key={id}>
      ...
      <motion.button
        ...
        className="transition-filter aspect-square rounded-lg border border-black/5 bg-neutral-100 saturate-90 brightness-90 duration-200 hover:saturate-100 hover:brightness-112 dark:border-white/5 dark:bg-neutral-800"
