...
export const GanttSidebarHeader: FC = () => (
  ...
  >
    ...
    <p className="flex-1 truncate text-start">Issues</p>
...
export const GanttSidebarGroup: FC<GanttSidebarGroupProps> = ({
  ...
  <div className={className}>
    <p
      className="w-full truncate p-2.5 text-start font-medium text-muted-foreground text-xs"
...
export const GanttSidebar: FC<GanttSidebarProps> = ({
  ...
  <div
    className={cn(
      "sticky start-0 z-30 h-max min-h-full overflow-clip border-border/50 border-e bg-background/90 backdrop-blur-md",
      className
    )}
...
export const GanttCreateMarkerTrigger: FC<GanttCreateMarkerTriggerProps> = ({
  ...
  return (
    <div
      className={cn(
        "group pointer-events-none absolute top-0 start-0 h-full w-full select-none overflow-visible",
        className
      )}
      ref={mouseRef}
    >
  ...
  return (
    <div
      className={cn(
        ...
        direction === "left" ? "-start-2.5" : "-end-2.5"
      )}
      ref={setNodeRef}
      {...attributes}
      {...listeners}
    >
      <div
        className={cn(
          ...
          direction === "left" ? "start-2.5" : "end-2.5",
          direction === "left" ? "group-hover:start-0" : "group-hover:end-0",
      ...
      {date && (
        <div
          className={cn(
            "-translate-x-1/2 rtl:translate-x-1/2 absolute top-10 hidden whitespace-nowrap rounded-lg border border-border/50 bg-background/90 px-2 py-1 text-foreground text-xs backdrop-blur-lg group-hover:block",
            isPressed && "block"
          )}
        >
...
export const GanttFeatureList: FC<GanttFeatureListProps> = ({
  ...
  <div
    className={cn("absolute top-0 start-0 h-full w-max space-y-4", className)}
...
> = memo(({ label, date, id, onRemove, className }) => {
  ...
  return (
    <div
      className="pointer-events-none absolute top-0 start-0 z-20 flex h-full select-none flex-col items-center justify-center overflow-visible"
      style={{
        width: 0,
